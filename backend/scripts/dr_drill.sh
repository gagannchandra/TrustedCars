...
docker-compose exec -T db pg_dump -U trustedcarz_user -d trustedcarz_db -F c -f /tmp/backup.dump
...
docker-compose exec -T db pg_restore -U trustedcarz_user -d trustedcarz_db --clean --if-exists -1 /tmp/backup.dump
